...
-   🤖 **AI-Ready Architecture**: The explicit workflow structure makes your codebase dramatically easier for LLMs and AI coding assistants to understand, navigate, and modify. See [AI & LLM Benefits](#ai--llm-benefits) for details.
...
When an LLM reads a Process class, it immediately understands the **complete business workflow** as an ordered list of steps:

```php
class OnboardNewCustomerProcess extends Process
{
    protected array $tasks = [
...
There's no need to trace through controllers, services, and repositories to understand what "onboarding a customer" means. The Process **is** the documentation. An LLM can read this and instantly answer: *"What happens when a new customer is onboarded?"* — something that would require navigating multiple files and method calls in a traditional architecture.

### Predictable Boundaries for Each Unit of Work

Each Task has a clear contract: it receives a payload, does **one thing**, and returns. The `@property-read` docblocks declare exactly what data flows in and out:
 ...
 */
class CreateAccount extends Task
{
    public function handle(): self
    {
...
- **Generate new Tasks** that follow the exact same pattern, reducing errors.
- **Understand data flow** between tasks — `$this->userId` set in one task is available to all subsequent tasks.
- **Suggest modifications** with confidence, knowing the blast radius is limited to a single task.
...
Brain eliminates these problems. The Process answers *"what"*, each Task answers *"how"*, and Queries handle *"read"* operations. This separation maps directly to how LLMs reason: break complex problems into smaller, well-defined steps.

### Practical Impact

| Scenario | Traditional Architecture | With Brain |
|----------|------------------------|------------|
| "Add a step to the checkout flow" | LLM must find the right service, understand the method call chain, and figure out where to inject new logic | LLM reads the Process, sees the task list, and adds a new Task in the right position |
| "What happens when a payment fails?" | LLM traces through controllers, services, event listeners, and exception handlers | LLM finds the Process and reads the ordered task list with `runIf()` conditions |
| "Reuse the email validation from user registration in the invite flow" | LLM must extract logic from a service method into something reusable | The Task already exists as a standalone, reusable unit — just add it to the new Process |
| "Which data does this workflow need?" | LLM reads multiple files to find all parameters | `@property-read` docblocks on each Task declare the full contract |

### Your Codebase as Context for AI

When you use `brain:show -vv`, the output gives a complete, structured map of your entire business logic — including processes, their sub-tasks, data inputs, and outputs. This output can be fed directly as context to an LLM, giving it a high-level understanding of your entire application in seconds.

In short: **the more structured your code, the better AI tools can help you write, review, and maintain it.** Brain makes your codebase AI-friendly by design.
...
>
> **The Process gives me an immediate map.** I look at the `$tasks` array and I know exactly what the workflow does, in what order, without tracing method calls across 5 different files. In a traditional service class, I have to read the entire method, follow every `$this->otherService->doSomething()`, understand what each one does, and build the mental map myself.
>
> **Each Task is an isolated unit with an explicit contract.** The `@property-read` annotations tell me what goes in, `@property` tells me what comes out. I don't need to *infer* — it's declared. This drastically reduces the chance of me suggesting code that breaks something I didn't see.
>
> **The pattern is repetitive — and that's great for me.** Every Task has `handle()`, returns `$this`, receives a payload. I can generate new Tasks with high confidence because the pattern is always the same. The more predictable the structure, the fewer mistakes I make.
>
> **Modifications have a small blast radius.** If you ask me to "add a step to the checkout flow," I don't need to touch a 200-line method and hope nothing breaks. I create a new Task and add it to the Process list. Simple, surgical.
>
> In practice, Brain's structure works as an **intermediate language between human intent and code**. The Process expresses the intent ("onboard a customer"), the Tasks express the execution ("create account", "send email"), and I can operate at both levels with clarity.
>
